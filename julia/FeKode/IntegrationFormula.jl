...
function IntegrationFormula(dim::Int64, order::Int64)
     if (dim == 0)
            return IntegrationFormula(0, [[0];]', [1.])
     elseif dim == 1 && order in 2:3
         return IntegrationFormula(1, [[(-sqrt(1/3.) + 1.0) / 2 (sqrt(1/3.) + 1.0) / 2];], [0.5, 0.5])
     elseif (dim,order) == (2,2)
         return IntegrationFormula(2, [[2/3. 1/6.]; [1/6. 1/6.]; [1/6. 2/3.]]', fill(1/6., 3))
     elseif (dim,order) == (2,3)
          begin inverse_tan = atan(0.75);
                ...
                return  IntegrationFormula(2, [[a b]; [a c]; [b a]; [b c]; [c a]; [c b] ]', fill(1/12., 6))
          end
     elseif (dim, order) == (3, 2)
         begin
             ...
             b = (1.0 - sqrtFifth) / 4.0;
             return IntegrationFormula(3,[[a b b]; [b a b]; [b b a]; [b b b]]' , fill(1/24., 4))
         end
...
using Base.Test
@test integrate(x->x[1]*x[1], IntegrationFormula(1,2)) ≈ 1./3 atol = 1e-12
@test integrate(prod,IntegrationFormula(2,2)) ≈ 1 ./ 24 atol=1.0e-12 # integrate(x*y,(y,0,1-x),(x,0,1))
@test integrate(x->x[1]^2*x[2], IntegrationFormula(2,3)) ≈ 1/60. atol= 1e-12 #integrate(x*x*y,(y,0,1-x),(x,0,1))
@test integrate(x->x[1]*x[2],IntegrationFormula(3,2)) ≈1/120. atol=1e-12 #integrate(x*y,(z,0,1-y-x),(y,0,1-x),(x,0,1))
@test integrate(prod,IntegrationFormula(3,3)) ≈ 1/720. atol= 1e-12 # integrate(x*y*z,(z,0,1-y-x),(y,0,1-x),(x,0,1))
